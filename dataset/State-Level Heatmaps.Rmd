...
normalize_opacity_dynamic <- function(value, min_value, max_value, min_opacity = 0.2, max_opacity = 1) {
...
server <- function(input, output, session) {
  ...
  merged_data <- left_join(states_sf, dataset, by = "state")

  # min and max values for POC_Proportion
  min_poc <- min(merged_data$POC_Proportion, na.rm = TRUE)
  max_poc <- max(merged_data$POC_Proportion, na.rm = TRUE)
  ...
  min_cases <- min(merged_data$total_cases, na.rm = TRUE)
  max_cases <- max(merged_data$total_cases, na.rm = TRUE)
